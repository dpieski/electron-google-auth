...
const fs = require('fs')
const clientSecretPath = path.join(`${__dirname}`, '/../assets', 'secrets', 'credentials.json')
...
async function loadTokens(callbackURL) {
	...
	try {
		...
		if (oAuth2Client.credentials) {
			...
			await keytar.setPassword(keytarService, keytarAccount, JSON.stringify(accessToken.tokens))
		}
		return true
	} catch (err) {
...
async function refreshTokens() {
	...
	if (credentials) {
		console.log('-Setting oAuth2Client credentials with credentials from keytar....')
